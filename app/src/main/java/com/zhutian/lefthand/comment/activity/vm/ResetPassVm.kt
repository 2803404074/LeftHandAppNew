...
class ResetPassVm : BaseViewModel() {
    var isSendSuccess = MutableLiveData<Boolean>()

    var resetPassStatus = MutableLiveData<Boolean>()
    ...
    fun resetPas(activity: Activity, phone: String, code: String, pwd: String) {
        if (!StringUtils.isEntity(phone)) {
        ...
        RequestUtil.resetPass(false,map, object : BaseObserver<Any>() {
            override fun onSuccess(result: Any?) {
                ...
                mo?.let {
                    ...
                    resetPassStatus.value = true
                }?:let {
                    resetPassStatus.value = false
                }
            }
